...
const productInventory = JSON.parse(readFileSync(
  new URL("../../../docs/soso-legacy-product-inventory.json", import.meta.url),
  "utf8",
));
const legacyProductRedirects = productInventory.products.map((product) => ({
  fromPath: new URL(product.sourceUrl).pathname,
  toPath: product.targetUrl,
  statusCode: 301,
}));
legacyRedirects.push(...legacyProductRedirects);
