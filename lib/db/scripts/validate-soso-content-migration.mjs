...
const requiredIndexes = [
  "soso_accessory_launch_notifications_identity_idx",
  "soso_accessory_launch_notifications_summary_idx",
  "soso_audit_logs_entity_id_created_idx",
  "soso_analytics_events_consent_occurred_idx",
  "soso_commerce_webhook_catalogue_event_idx",
...
const expectedFirstRunFiles = [
  ...
  "0006_soso_order_item_colour_snapshot.sql",
  "0007_accessory_launch_notifications.sql",
  "0007_justicesure_v33_checkout_durability.sql",
  "0008_accessory_launch_notification_summary_index.sql",
  "0008_restore_marketing_pixel_checks.sql",
  "0009_justicesure_payment_attempt_recovery.sql",
  "0010_justicesure_payment_session_identifiers.sql",
  "0011_justicesure_catalogue_webhook_invalidation.sql",
];
...
try {
  ...
  await client.query(`
    INSERT INTO soso_accessory_launch_notifications (
      email,
      product_slug,
      accessory_category,
    ...
    SELECT
      CASE WHEN series % 2 = 0
        THEN 'SHOPPER-' || series || '@EXAMPLE.COM'
        ELSE 'shopper-' || series || '@example.com'
      END,
      'accessory-' || (series % 40),
      'category-' || (series % 8),
      true,
      timestamptz '2026-09-14 12:00:00+00'
        - ((series % 120) || ' days')::interval
    FROM generate_series(1, 100000) AS series
  `);
  await client.query("ANALYZE soso_accessory_launch_notifications");
  const summaryPlan = await client.query(`
    ...
    SELECT
      ...
      count(DISTINCT lower(email)) FILTER (
        WHERE created_at < timestamptz '2026-09-01 00:00:00+00'
      ) AS previous_request_count
    FROM soso_accessory_launch_notifications
    WHERE created_at >= timestamptz '2026-08-19 00:00:00+00'
      AND created_at <= timestamptz '2026-09-14 23:59:59.999+00'
    GROUP BY accessory_category, product_slug
  `);
  const summaryPlanJson = JSON.stringify(summaryPlan.rows[0]["QUERY PLAN"]);
  const summaryExecutionMs =
    summaryPlan.rows[0]["QUERY PLAN"][0]["Execution Time"];
  assert.match(
    summaryPlanJson,
    /soso_accessory_launch_notifications_summary_idx/,
  );
  assert.ok(
    summaryExecutionMs < 2000,
    `Expected 100k-row accessory summary under 2000ms, got ${summaryExecutionMs}ms`,
  );
  ...
  assert.deepEqual(customColumns.rows, [
    ...
    { table_name: "soso_order_items", column_name: "custom_colour" },
    { table_name: "soso_order_items", column_name: "line_number" },
    { table_name: "soso_order_items", column_name: "selected_colour_hex" },
    { table_name: "soso_order_items", column_name: "selected_colour_id" },
    { table_name: "soso_order_items", column_name: "selected_colour_label" },
  ...
  process.stdout.write(
    `Validated fresh initialization, repeat execution, legacy upgrade, partial-schema refusal, and 100k-row accessory summary in ${summaryExecutionMs.toFixed(2)}ms\n`,
  );
} finally {
