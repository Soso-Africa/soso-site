...
**How to apply:** Reconcile readable durable consent immediately before optional events, keep an in-memory fallback only when storage cannot be read, and maintain module-level guards for the current consent grant/path and the once-per-session start marker. Private route classification must remain shared between the client shell and server validator. For an apparently missing live visit, first confirm the Vercel storefront URL and affirmative consent, then manually refresh the Staff dashboard because auto-refresh defaults to Off; this sequence was confirmed to surface newly accepted production events.