...
// I think the issue is the missing closing tag in the file somewhere! 
// Let's rewrite the whole file to guarantee syntax is correct.
...
export function MaterialTurnStage({ sets, productCopy }: MaterialTurnStageProps) {
  ...
  return (
    <div ref={trackRef} className="relative w-full" style={{ height: \`\${totalStates * 100}vh\` }}>
      <div 
        ref={stickyRef} 
        className="sticky top-[80px] w-full h-[calc(100vh-140px)] min-h-[600px] md:h-[calc(100vh-120px)] bg-background overflow-hidden flex flex-col group"
      >
        <div aria-live="polite" className="sr-only">
          Showing material {activeSet.label}, {activeView} view
        </div>
        ...
        <div className="flex-1 relative" style={{ perspective: "1500px", transformStyle: "preserve-3d" }}>
                ...
                {(["front", "back"] as const).map((view) => (
                   ...
                   >
                     <img
                       src={set[view].src}
                       alt={set[view].alt}
                       className="absolute inset-0 w-full h-full object-cover origin-center transition-transform duration-500 ease-out"
           ...
           <div className="absolute inset-0 pointer-events-none z-20 flex flex-col justify-between p-4">
              <div className="flex justify-between items-start">
                <div className="text-[10px] tracking-[0.25em] uppercase px-3 py-1.5 pointer-events-auto bg-white/90 text-foreground backdrop-blur-sm border border-border transition-opacity">
                  {activeSet.label}
                </div>
              </div>
              ...
              <div className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col gap-2 pointer-events-auto">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-background/80 text-foreground backdrop-blur-sm border border-border disabled:opacity-40 transition-opacity"
              ...
              <div className="flex justify-between items-end">
                ...
                <button
                  ...
                  className="flex min-h-10 min-w-10 items-center justify-center bg-white/90 text-foreground backdrop-blur-sm border border-border pointer-events-auto transition-transform active:scale-95"
        ...
        <div className="flex gap-3 mt-3 overflow-x-auto pb-2 snap-x hide-scrollbar px-1 pointer-events-auto">
          {sets.map((set, i) => (
            <button
              ...
              className="w-16 shrink-0 snap-start overflow-hidden relative group"
              style={{
            ...
            >
              <img src={set.front.src} alt={set.front.alt} className="aspect-[3/4] object-cover w-full group-hover:scale-105 transition-transform duration-500" />
              {i !== activeIndex && (
