...
/** References outside the target product, including absolute links that schema checks allow. */
export function platformProductReferences(content: PlatformContent, slug: string): string[] {
  ...
  const visit = (value: unknown, path: string): void => {
    if (typeof value === "string") {
      ...
      catch { /* Unparseable text is not a product link. */ }
      if (value === slug || isProductUrl) references.push(path);
